...
def sinc_filter(cutoff: torch.Tensor, window_size: int = 512, high_pass: bool = False):
    """Create windowed-sinc filter for given low-pass cutoff frequencies.
    ...
    filt = filt * torch.hamming_window(window_size, device=device, dtype=dtype).unsqueeze(0)
    filt = filt / filt.sum(axis=-1, keepdims=True).abs()
...
def apply_time_varying_filter(waveform: torch.Tensor, filters: torch.Tensor, delay_compensation: int = -1):
    """Applies distinct filters along time axis of the given waveform.
    ...
    3. Placing the filtered chunks at the original indices while adding up the overlapping parts.
    ...
    Args:
        ...
        filters (Tensor):
            Shape `[num_filters, filter_length]` or `[..., num_filters, filter_length]`.
            If 2D, the same filter set is applied to all the signals.
            Otherwise, filter sets can be different for channel, batch and other dimensions.
    ...
    if filters.ndim not in [2, waveform.ndim + 1]:
        raise ValueError(f"`filters` must be 2D or N+1 D where N is the dimension of waveform. Found: {filters.ndim}")
